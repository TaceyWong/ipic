    ...
    <header class="border-b-2 border-gray-800 flex flex-wrap" ref="header">
      <div class="w-full md:w-1/2 lg:w-5/12">
        ...
        >
          ...
          <p class="mt-6 text-gray-200">选择文件或将文件拖拽至此处</p>
        </div>
      </div>
        ...
        <table class="table-auto w-full text-left whitespace-no-wrap">
            ...
            <tr>
              <th
                class="
                  ...
                  pl-3
                  pt-6
                  text-gray-400 text-left text-sm
                  tracking-wider
                  uppercase
                "
              >
              ...
              <th
                class="
                  ...
                  pl-3
                  pt-6
                  text-center text-gray-400 text-left text-sm
                  tracking-wider
                  uppercase
                "
              >
          ...
          <tbody>
            <tr v-for="(file, i) in files" :key="`${i}-${file.name}`">
              ...
              <td>
                <p class="cell-r flex items-center justify-center">
                  <button
                    class="mbtn focus:outline-none ta-slow"
                    :class="
                      file.isConverted
                        ? 'border-green hover:bg-green hover:text-gray-900 text-gray-200'
                        : 'btn--disabled'
...
export default {
  ...
  methods: {
    ...
    /**
     * delete opt picture file
     */
    del() {
      window.backend.FileManager.Delete()
        .then((res) => {
          console.log(res)
        })
        .catch((err) => {
      ...
      f.forEach((f) => {
        ...
        if (!type) {
          EventBus.$emit('notify', {
            msg: `文件类型不支持. 合法文件为 JPG, PNG, 和 WebP.`,
            type: 'warn',
          })
          return
        }
  ...
  mounted() {
    ...
    Wails.Events.On('conversion:stat', (e) => {
      ...
      if (r > 0) {
        EventBus.$emit('notify', {
          msg: `优化图片并调整尺寸:${c},耗时:${prettyTime(t)[0]} ${prettyTime(
            t
          )[1].toLowerCase()}.`,
        })
      } else {
        EventBus.$emit('notify', {
          msg: `优化图片:${c}，耗时:${prettyTime(t)[0]} ${prettyTime(
            t
          )[1].toLowerCase()}.`,
        })
      }
